 ...
 * @author admin
 */
@WebServlet(name = "ReturnOrderServlet", urlPatterns = {"/ReturnOrderServlet"})
public class ReturnOrderServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (account != null) {
            ...
            try {
                boolean updateStatus = dao.updateOrderStatus(orderID, "Trả Lại Hàng");
                boolean returnQuantity;
                for (int i = 0; i < productIDs.length; i++) {
                    int id = Integer.parseInt(productIDs[i]);
                    int num = Integer.parseInt(numberOfProducts[i]);
                    returnQuantity = dao.inceaseProductStoreQuantity(id, num);
                }
            } catch (Exception e) {
            } finally {
